...
function CrudResource({
  ...
  const columns: ColumnsType<any> = useMemo(() => {
    ...
    const actionsCol: any = {
      title: '', key: 'actions', width: 120, fixed: 'right', align: 'center',
      children: [{
        title: (<Tooltip title="重置所有搜索"><Button size="small" icon={<ReloadOutlined />} onClick={resetFilters} /></Tooltip>),
        key: 'actions_child', width: 120, fixed: 'right', align: 'center',
        render: (_: any, record: any) => (
          <Space size={4}>
            <Tooltip title="查看"><Button size="small" type="text" icon={<EyeOutlined />} onClick={() => openView(record)} /></Tooltip>
            {canUpdate && (<Tooltip title="修改"><Button size="small" type="text" icon={<EditOutlined />} onClick={() => openEdit(record)} /></Tooltip>)}
            {canDelete && (
...
function RoutesSubTab(props: SubTabProps) {
  const columns: CrudColumn[] = [
    { key: 'route_name', title: '航线名称', width: 160, searchable: true, sortable: true, render: (v) => <Tag color="blue">{v}</Tag> },
    { key: 'route_code', title: '航线代码', width: 120, searchable: true },
    { key: 'carrier_type', title: '承运类型', width: 110, searchable: true, sortable: true, render: (v: string) => v ? <Tag color={carrierTypeColor(v)}>{v}</Tag> : '' },
    { key: 'carrier_tool_name', title: '承运工具名称', width: 140, searchable: true },
    { key: 'carrier', title: '承运人', width: 140, searchable: true },
    { key: 'departure_port', title: '起运港', width: 150, searchable: true, ellipsis: true },
    { key: 'destination_port', title: '目的港', width: 150, searchable: true, ellipsis: true },
    { key: 'is_enabled', title: '是否启用', width: 100, filter: 'enabled', sortable: true, render: (v: number) => <Tag color={v === 1 ? 'success' : 'default'}>{v === 1 ? '启用' : '停用'}</Tag> },
    { key: 'description', title: '备注', width: 160, searchable: true },
    { key: 'created_at', title: '创建时间', width: 180, filter: 'date', sortable: true, render: (v) => formatDateTime(v) },
    { key: 'updated_at', title: '更新时间', width: 180, filter: 'date', sortable: true, render: (v) => formatDateTime(v) },
  ];
  return (
    <CrudResource
...
function ContainersSubTab(props: SubTabProps) {
  const columns: CrudColumn[] = [
    { key: 'container_no', title: '箱号', width: 160, searchable: true, sortable: true, render: (v) => <Tag color="purple">{v}</Tag> },
    { key: 'container_type', title: '箱型', width: 140, searchable: true, sortable: true },
    { key: 'is_enabled', title: '是否启用', width: 100, filter: 'enabled', sortable: true, render: (v: number) => <Tag color={v === 1 ? 'success' : 'default'}>{v === 1 ? '启用' : '停用'}</Tag> },
...
function VoyagesSubTab(props: SubTabProps) {
  ...
  const fetchRouteOptions = async () => {
    try {
      const res = await adminFetch('/admin/ship-routes/options');
  ...
  const columns: CrudColumn[] = [
    { key: 'voyage_name', title: '班(航)次名称', width: 150, searchable: true, sortable: true, render: (v) => <Tag color="blue">{v}</Tag> },
    { key: 'voyage_no', title: '班(航)次号', width: 120, searchable: true, sortable: true },
    { key: 'route_name', title: '航线', width: 140, ellipsis: true, render: (v) => v || '—' },
    { key: 'etd', title: '预计启航', width: 165, filter: 'date', sortable: true, render: (v) => formatDateTime(v) },
    { key: 'eta', title: '预计到港', width: 165, filter: 'date', sortable: true, render: (v) => formatDateTime(v) },
    { key: 'atd', title: '实际启航', width: 165, sortable: true, render: (v) => formatDateTime(v) },
    { key: 'ata', title: '实际到港', width: 165, sortable: true, render: (v) => formatDateTime(v) },
    { key: 'departure_port', title: '起运港', width: 130, searchable: true },
    { key: 'destination_port', title: '目的港', width: 130, searchable: true },
    { key: 'si_cutoff', title: '截单时间', width: 165, render: (v) => formatDateTime(v) },
    { key: 'cargo_cutoff', title: '交货截止', width: 165, render: (v) => formatDateTime(v) },
    { key: 'vgm_cutoff', title: '截VGM时间', width: 165, render: (v) => formatDateTime(v) },
    { key: 'is_enabled', title: '是否启用', width: 100, filter: 'enabled', sortable: true, render: (v: number) => <Tag color={v === 1 ? 'success' : 'default'}>{v === 1 ? '启用' : '停用'}</Tag> },
    { key: 'description', title: '备注', width: 150, searchable: true },
    { key: 'created_at', title: '创建时间', width: 180, filter: 'date', sortable: true, render: (v) => formatDateTime(v) },
    { key: 'updated_at', title: '更新时间', width: 180, filter: 'date', sortable: true, render: (v) => formatDateTime(v) },
  ];
  return (
    <CrudResource
      ...
      providerOptions={props.providerOptions}
      refreshKey={props.refreshKey}
      searchPlaceholder="搜索班(航)次：名称、班次号、起运港/目的港或航线"
      createDefaults={{ is_enabled: true }}
...
function BillsSubTab(props: SubTabProps) {
  ...
  const fetchAux = async () => {
    ...
    try {
      const [vRes, cRes, sRes] = await Promise.all([
        adminFetch('/admin/ship-voyages/options'),
  ...
  const columns: CrudColumn[] = [
    ...
    { key: 'seal_no', title: '封条号', width: 110, searchable: true },
    { key: 'package_count', title: '件数', width: 80, render: (v) => v ?? '' },
    { key: 'weight', title: '重量', width: 90, render: (v) => v ?? '' },
    { key: 'volume', title: '体积', width: 90, render: (v) => v ?? '' },
    { key: 'marks', title: '唛头', width: 120, searchable: true },
    { key: 'delivery_place', title: '交货地', width: 120, searchable: true },
    { key: 'cargo_status', title: '货物状态', width: 120, render: (v: string) => v ? <Tag color="processing">{cargoStatusMap[v] || v}</Tag> : '' },
    { key: 'description', title: '备注', width: 150, searchable: true },
    { key: 'created_at', title: '创建时间', width: 180, filter: 'date', sortable: true, render: (v) => formatDateTime(v) },
    { key: 'updated_at', title: '更新时间', width: 180, filter: 'date', sortable: true, render: (v) => formatDateTime(v) },
  ];
...
export default function RouteTransportTab({ actorScope, canCreate, canUpdate, canDelete, refreshKey }: RouteTransportTabProps) {
  ...
  const subProps: SubTabProps = { showProviderSelect, providerOptions, canCreate, canUpdate, canDelete, refreshKey };
